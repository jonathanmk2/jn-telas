...
export function VsPhonePricing({ products, isLoggedIn }: { products: Product[]; isLoggedIn: boolean }) {
    ...
    startTransition(async () => {
      try {
        ...
        if (response.ok && data.ok) {
          keyRef.current = null
          setPayment({ orderId: data.orderId, qrCode: data.qrCode ?? null, qrCodeBase64: data.qrCodeBase64 ?? null })
          setStatus('pending')
          setCopied(false)
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        const response = await fetch(`/api/payment-status?orderId=${payment!.orderId}`, { cache: 'no-store' })
        ...
        <div className="mx-auto max-w-2xl px-3 sm:px-4">
          <div className="relative mx-auto max-w-lg rounded-2xl border border-primary bg-card p-4 shadow-xl shadow-primary/15 sm:p-5">
            ...
            <div className="pt-3 text-center">
              ...
              <h3 className="text-xl font-bold">VSPHONE VIP</h3><p className="mt-0.5 text-xs font-semibold text-primary">30 DIAS</p>
            ...
            <div className="mt-2 grid grid-cols-3 gap-1 rounded-lg border bg-secondary/40 p-2 text-center text-[10px]"><div className={quantity < 5 ? 'font-bold text-primary' : 'text-muted-foreground'}><div>1–4</div><div>R$ 1 TESTE</div></div><div className={quantity >= 5 && quantity < 10 ? 'font-bold text-primary' : 'text-muted-foreground'}><div>5–9</div><div>R$ 23</div></div><div className={quantity >= 10 ? 'font-bold text-primary' : 'text-muted-foreground'}><div>10+</div><div>R$ 22</div></div></div>

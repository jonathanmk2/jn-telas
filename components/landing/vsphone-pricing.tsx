...
export function VsPhonePricing({ products, isLoggedIn }: { products: Product[]; isLoggedIn: boolean }) {
  ...
  return <>
    <section className="bg-secondary/30 pb-5 pt-2"><div className="mx-auto max-w-2xl px-3 sm:px-4"><div className="relative mx-auto max-w-lg rounded-2xl border border-primary bg-card p-4 shadow-xl shadow-primary/15 sm:p-5">
      ...
      <div className="pt-3 text-center"><div className="group relative mx-auto mb-2 flex h-16 w-40 items-center justify-center overflow-hidden rounded-2xl border border-primary/35 bg-gradient-to-br from-primary/15 via-background to-cyan-400/10 shadow-[0_0_28px_rgba(117,103,255,0.25)]"><div className="absolute inset-0 animate-pulse bg-[radial-gradient(circle_at_50%_50%,rgba(117,103,255,0.18),transparent_65%)]" /><div className="absolute -left-10 top-0 h-full w-8 -skew-x-12 bg-white/20 blur-sm animate-[vsphoneShine_2.8s_ease-in-out_infinite]" /><div className="relative flex items-center gap-2 animate-[vsphoneFloat_3s_ease-in-out_infinite]"><span className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-cyan-400 text-sm font-black italic text-white shadow-[0_0_18px_rgba(34,211,238,0.45)]">VS</span><span className="text-lg font-black tracking-tight"><span className="text-primary">VS</span>PHONE</span></div></div><style jsx>{`@keyframes vsphoneFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-3px) scale(1.03)}}@keyframes vsphoneShine{0%,35%{transform:translateX(-30px) skewX(-12deg);opacity:0}50%{opacity:1}75%,100%{transform:translateX(220px) skewX(-12deg);opacity:0}}@media (prefers-reduced-motion:reduce){.animate-pulse{animation:none!important}}`}</style><h3 className="text-xl font-bold">VSPHONE VIP</h3><p className="mt-0.5 text-xs font-semibold text-primary">30 DIAS</p><p className="mt-1 text-[10px] text-muted-foreground sm:text-xs">Código de ativação • Entrega automática após o PIX</p><div className="mt-2 flex items-baseline justify-center gap-1"><span className="text-3xl font-bold sm:text-4xl">{formatBRL(price)}</span><span className="text-xs text-muted-foreground">/ tela</span></div></div>

...
export function VsPhonePricing({ products, isLoggedIn }: { products: Product[]; isLoggedIn: boolean }) {
  ...
  return <>
    <section className="bg-secondary/30 pb-5 pt-2"><div className="mx-auto max-w-2xl px-3 sm:px-4"><div className="relative mx-auto max-w-lg rounded-2xl border border-primary bg-card p-4 shadow-xl shadow-primary/15 sm:p-5">
      ...
      <div className="pt-3 text-center"><div className="relative mx-auto mb-3 flex h-[112px] w-full max-w-sm items-center justify-center overflow-hidden rounded-[24px] border-2 border-amber-300 bg-zinc-950 px-5 shadow-[0_0_34px_rgba(245,158,11,0.38),inset_0_0_24px_rgba(245,158,11,0.10)]"><div className="absolute -left-10 top-1/2 h-[72px] w-[440px] -translate-y-1/2 rounded-[50%] border border-amber-400/90 shadow-[0_0_12px_rgba(245,158,11,0.8)] animate-[vsOrbit_4s_ease-in-out_infinite]" /><div className="absolute -left-10 top-0 h-full w-10 -skew-x-12 bg-amber-100/40 blur-md animate-[vsphoneShine_2.8s_ease-in-out_infinite]" /><div className="relative z-10 flex items-center gap-3 animate-[vsphoneFloat_3s_ease-in-out_infinite]"><img src="/vsphone-symbol.svg" alt="VSPhone" className="h-[78px] w-[112px] scale-[1.12] object-contain brightness-125 contrast-125 mix-blend-lighten drop-shadow-[0_0_16px_rgba(245,158,11,0.95)]" /><div className="text-[30px] font-black italic tracking-[-0.05em]"><span className="bg-gradient-to-b from-yellow-200 via-amber-400 to-orange-500 bg-clip-text text-transparent">VS</span><span className="text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.25)]">PHONE</span></div></div><div className="absolute bottom-3 left-1/2 h-px w-2/3 -translate-x-1/2 bg-gradient-to-r from-transparent via-amber-300 to-transparent shadow-[0_0_10px_rgba(245,158,11,1)]" /></div><style jsx>{`@keyframes vsphoneFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-2px) scale(1.02)}}@keyframes vsphoneShine{0%,30%{transform:translateX(-50px) skewX(-12deg);opacity:0}48%{opacity:1}72%,100%{transform:translateX(440px) skewX(-12deg);opacity:0}}@keyframes vsOrbit{0%,100%{transform:translateY(-50%) rotate(-4deg) scaleX(1)}50%{transform:translateY(-50%) rotate(4deg) scaleX(.94)}}`}</style><h3 className="text-xl font-bold">VSPHONE VIP</h3><p className="mt-0.5 text-xs font-semibold text-primary">30 DIAS</p><p className="mt-1 text-[10px] text-muted-foreground sm:text-xs">Código de ativação • Entrega automática após o PIX</p><div className="mt-2 flex items-baseline justify-center gap-1"><span className="text-3xl font-bold sm:text-4xl">{formatBRL(price)}</span><span className="text-xs text-muted-foreground">/ tela</span></div></div>

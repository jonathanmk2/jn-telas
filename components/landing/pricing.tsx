...
function LdCloudMascot() {
  return (
    <div className="relative mx-auto flex h-28 w-32 shrink-0 -translate-x-3 -translate-y-2 items-end justify-center sm:h-32 sm:w-36 sm:translate-x-0 sm:translate-y-0" aria-label="Mascote LD Cloud">
...
export function Pricing({ products, isLoggedIn }: { products: Product[]; isLoggedIn: boolean }) {
  ...
  return (
    <>
      ...
      {payment && <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 p-4 py-6 sm:items-center sm:py-4">
        <div className="relative my-auto w-full max-w-md rounded-2xl bg-background p-5 shadow-2xl sm:p-6">
          <button type="button" onClick={closePaymentModal} className="absolute right-3 top-3 rounded-full p-2 text-muted-foreground hover:bg-secondary" aria-label="Fechar"><X className="size-5" /></button>
          {paymentConfirmed ? <div className="py-5 text-center"><CheckCircle2 className="mx-auto size-20 text-green-500" /><h2 className="mt-5 text-2xl font-bold">Pagamento confirmado!</h2><p className="mt-2 text-muted-foreground">Seu pagamento foi aprovado com sucesso.</p><div className="mt-6 flex flex-col gap-2 sm:flex-row"><Button className="flex-1" onClick={() => window.location.href = `/minha-conta?updated=${Date.now()}`}>Ver meus códigos</Button><Button variant="outline" className="flex-1" onClick={closePaymentModal}>Fechar</Button></div></div> : orderStatus === 'cancelled' ? <div className="py-5 text-center"><div className="mx-auto flex size-20 items-center justify-center rounded-full bg-destructive/10 text-destructive"><X className="size-10" /></div><h2 className="mt-5 text-2xl font-bold">Pagamento cancelado</h2><p className="mt-2 text-muted-foreground">Este pagamento não foi concluído.</p><Button className="mt-6 w-full" onClick={closePaymentModal}>Fechar</Button></div> : <div className="text-center"><h2 className="text-2xl font-bold">Pague com PIX</h2><p className="mt-2 text-sm text-muted-foreground">Escaneie o QR Code ou copie o código PIX.</p>{qrImage && <img src={qrImage} alt="QR Code PIX" className="mx-auto mt-5 size-64 max-w-full rounded-xl border bg-white p-2" />}{payment.qrCode && <div className="mt-4"><p className="mb-2 text-left text-xs font-semibold text-muted-foreground">Código PIX</p><div className="max-h-28 overflow-auto break-all rounded-xl border bg-secondary/30 p-3 text-left font-mono text-xs leading-5">{payment.qrCode}</div><Button type="button" className="mt-3 h-10 w-full" variant={pixCopied ? 'secondary' : 'default'} onClick={copyPixCode}><Copy className="size-4" />{pixCopied ? 'PIX copiado com sucesso!' : 'Copiar código PIX'}</Button></div>}<div className="mt-5 rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-3"><p className="text-sm font-semibold text-yellow-700 dark:text-yellow-300">{checkingPayment ? 'Aguardando confirmação do pagamento...' : 'Após o pagamento, a confirmação será automática.'}</p><p className="mt-1 text-xs text-muted-foreground">Não feche esta janela até o pagamento ser confirmado.</p></div></div>}
        </div>
      </div>}
    </>
  )

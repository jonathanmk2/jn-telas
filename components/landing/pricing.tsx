      ...
      <section id="planos" className="scroll-mt-20 bg-secondary/30 py-3 sm:py-5">
        <div className="mx-auto max-w-2xl px-3 sm:px-4">
          {product && <div className="relative mx-auto max-w-lg rounded-2xl border border-primary bg-card p-4 shadow-xl shadow-primary/15 sm:p-5">
            ...
            {rateLimitMessage && <div role="alert" className="mt-2 rounded-lg border border-red-500/40 bg-red-500/10 p-2 text-center"><p className="text-xs font-semibold text-red-500">🚫 Limite de pedidos atingido</p><p className="text-[10px] text-muted-foreground">{rateLimitMessage}</p></div>}
            {stockMessage && <div role="alert" className="mt-2 rounded-lg border border-yellow-500/40 bg-yellow-500/10 p-3 text-center"><p className="text-xs font-semibold text-yellow-600 dark:text-yellow-400">⚠️ Estoque insuficiente</p><p className="mt-1 text-[10px] text-muted-foreground">{stockMessage}</p><a href="https://wa.me/5585985373629?text=Ol%C3%A1%2C%20tentei%20comprar%20telas%20na%20JN%20TELAS%2C%20mas%20apareceu%20um%20alerta%20de%20estoque%20insuficiente.%20Gostaria%20de%20saber%20quando%20haver%C3%A1%20disponibilidade." target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex h-9 w-full items-center justify-center gap-2 rounded-md bg-green-600 px-4 text-sm font-medium text-white transition-colors hover:bg-green-700"><MessageCircle className="size-4" />Falar no WhatsApp</a></div>}
          </div>}
        </div>
      </section>

      {payment && <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/70 p-3 sm:p-4">
        <div className="relative my-auto w-full max-w-md max-h-[calc(100dvh-1rem)] overflow-y-auto rounded-2xl bg-background p-4 shadow-2xl sm:max-h-[calc(100dvh-2rem)] sm:p-6">
          <button type="button" onClick={closePaymentModal} className="absolute right-3 top-3 z-10 rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Fechar"><X className="size-5" /></button>
          {paymentConfirmed ? <div className="py-4 text-center sm:py-6">
            <CheckCircle2 className="mx-auto size-16 text-green-500 sm:size-20" /><h2 className="mt-4 text-2xl font-bold">Pagamento confirmado!</h2><p className="mt-3 text-sm text-muted-foreground">Seu pagamento foi aprovado com sucesso.</p>
            <div className="mt-4 rounded-xl border border-green-500/30 bg-green-500/10 px-3 py-2 text-xs text-green-700 dark:text-green-400">Pedido atualizado automaticamente. Seus códigos já estão disponíveis em Minha Conta.</div>
            <Button className="mt-6 w-full" onClick={closePaymentModal}>Ver meus códigos</Button><Button variant="outline" className="mt-3 w-full" onClick={closePaymentModal}>Fechar</Button>
          </div> : orderStatus === 'cancelled' ? <div className="py-6 text-center"><h2 className="text-2xl font-bold">Pagamento cancelado</h2><p className="mt-3 text-sm text-muted-foreground">Este pagamento não foi aprovado.</p><Button variant="outline" className="mt-8 w-full" onClick={closePaymentModal}>Fechar</Button></div> : <div className="text-center">
            <h2 className="text-xl font-bold sm:text-2xl">Pagamento PIX</h2><p className="mt-2 text-sm text-muted-foreground">Escaneie o QR Code ou copie o código PIX para concluir o pagamento.</p>
            {qrImage ? <div className="mt-4 flex justify-center sm:mt-5"><div className="rounded-xl bg-white p-2.5 sm:p-3"><img src={qrImage} alt="QR Code PIX" className="aspect-square h-auto w-full max-w-[250px] object-contain sm:max-w-[270px]" /></div></div> : <div className="mt-4 rounded-xl border border-yellow-500/40 bg-yellow-500/10 p-4 text-sm">O pagamento foi criado, mas o QR Code não foi encontrado na resposta.</div>}
            {payment.qrCode && <div className="mt-4"><p className="mb-2 text-left text-xs font-semibold text-muted-foreground">Código PIX</p><div className="max-h-32 overflow-y-auto rounded-xl border bg-muted/30 p-3 text-left text-[11px] leading-relaxed break-all select-all">{payment.qrCode}</div><Button className="mt-3 w-full" onClick={copyPixCode}>{pixCopied ? <CheckCircle2 className="size-4" /> : <Copy className="size-4" />}{pixCopied ? 'PIX copiado com sucesso!' : 'Copiar código PIX'}</Button></div>}
            ...
            <Button variant="outline" className="mt-3 w-full" onClick={closePaymentModal}>Fechar</Button>
          </div>}
        </div>
      </div>}
    </>
